(* Minimal oriented graded posets with embeddings.
   Internal: no well-formedness checks. Efficient and replaceable IntSet backend. *)
...
let sizes x =
  Array.init (x.dim + 1) (fun d -> Array.length x.faces_in.(d))
...
let faces_of sign x ~dim ~pos =
  match sign with
  | `Input  -> x.faces_in.(dim).(pos)
  | `Output -> x.faces_out.(dim).(pos)
  | `Both   -> IntSet.union x.faces_in.(dim).(pos) x.faces_out.(dim).(pos)

let cofaces_of sign x ~dim ~pos =
  match sign with
  | `Input  -> x.cofaces_in.(dim).(pos)
  | `Output -> x.cofaces_out.(dim).(pos)
  | `Both   -> IntSet.union x.cofaces_in.(dim).(pos) x.cofaces_out.(dim).(pos)
...
let disjoint_union b c : t * inj * inj =
  ...
  let merge_faces fb fc =
    Array.init dims (fun d ->
      ...
      Array.init n (fun k ->
        if k < szb.(d)
        then set_map (shift_down d) fb.(d).(k)
  ...
  and merge_cofaces cb cc =
    ...
    Array.init dims (fun d ->
      ...
      Array.init n (fun k ->
        if k < szb.(d)
        then set_map (shift_up_co d) cb.(d).(k)
...
module UF = struct
  ...
  let rec find uf x =
    ...
    if p = x then x else let r = find uf p in uf.parent.(x) <- r; r
  let union uf a b =
  ...
  let rep_to_new =
    Array.init dims (fun d ->
      ...
      Array.init sz_u.(d) (fun i ->
        ...
        | Some k -> k
        | None -> let k = !next in incr next; Hashtbl.add tbl r k; k))
  in
  let sz_q =
    Array.init dims (fun d ->
      let maxk = ref (-1) in
  ...
  let classes =
    Array.init dims (fun d ->
      ...
      for i = 0 to sz_u.(d) - 1 do
        let k = rep_to_new.(d).(i) in buckets.(k) := i :: !(buckets.(k))
      done;
...
let pushout i j =
  ...
  for d = 0 to Array.length sz_a - 1 do
    for x = 0 to sz_a.(d) - 1 do
      let b_ix = i.Embedding.map.(d).(x)
      and c_ix = j.Embedding.map.(d).(x) in
      let u_b = inj_b.map.(d).(b_ix)
      and u_c = inj_c.map.(d).(c_ix) in
      UF.union ufs.(d) u_b u_c
    done
  done;
  let (p, qmap) = quotient u ufs in
  let map_b = Array.mapi (fun d arr -> Array.map (fun i -> qmap.(d).(i)) arr) inj_b.map
  and map_c = Array.mapi (fun d arr -> Array.map (fun i -> qmap.(d).(i)) arr) inj_c.map in
...
let coequaliser f g =
  ...
  let map_a =
    Array.mapi (fun d arrf -> Array.map (fun i -> qmap.(d).(i)) arrf) f.Embedding.map
  in
...
let boundary (sign : sign) (at_dim : int) (x : t) : t * Embedding.t =
  let dmax = x.dim in
  if dmax <= 0 then
    let e = { dim = -1; faces_in=[||]; faces_out=[||];
              cofaces_in=[||]; cofaces_out=[||] } in
    (e, Embedding.id e)
  else
    let dims = dmax + 1 in
    ...
    for i = 0 to n_d - 1 do
      if IntSet.is_empty (cofaces_of sign x ~dim:at_dim ~pos:i)
      then keep.(at_dim) <- IntSet.add i keep.(at_dim)
    done;
    (* 2) downward closure via both face orientations *)
    let stack = ref (List.map (fun i -> (at_dim, i)) (IntSet.elements keep.(at_dim))) in
    while !stack <> [] do
      match !stack with
      | [] -> ()
      | (d,i)::rest ->
    ...
    let new_index =
      Array.mapi (fun d _ ->
        ...
        IntSet.iter (fun old -> a.(old) <- !k; incr k) keep.(d); a
      ) x.faces_in
    in
    ...
    let old_of_new =
      Array.mapi (fun d _ ->
        ...
        IntSet.iter (fun old ->
          ...
          if ni >= 0 then inv.(ni) <- old) keep.(d);
        inv
      ) x.faces_in
    in
    let remap_lower src dd =
      set_filter_map (fun j ->
        let jj = new_index.(dd-1).(j) in if jj = -1 then None else Some jj) src
...
let boundary_top (s : sign) (x : t) : t * Embedding.t =
  boundary s (dim x - 1) x
